...
export default function FlowAnimation() {
  ...
  return (
    <div className="mt-16 w-full max-w-4xl relative h-[360px] hidden md:flex items-center justify-center animate-[float_6s_ease-in-out_infinite]">
      {/* Router Hub - Center */}
      <div className="landing-brand-flow-core relative z-20 w-32 h-32 rounded-full bg-[#23180f] border-2 flex flex-col items-center justify-center gap-1 group cursor-pointer hover:scale-105 transition-transform duration-500">
        <BrandLogo className="size-10" iconClassName="text-4xl" decorative />
        <span className="max-w-24 truncate text-xs font-bold text-white tracking-widest uppercase" data-i18n-skip="true">
          {branding.name}
        </span>
        <div className="landing-brand-border absolute inset-0 rounded-full border animate-ping opacity-20"></div>
      </div>
      ...
      <div className="absolute left-0 top-1/2 -translate-y-1/2 flex flex-col gap-7">
        {CLI_TOOLS.map((tool) => (
          ...
          >
            <div className="landing-brand-hover-border w-16 h-16 rounded-2xl bg-[#23180f] border border-[#3a2f27] flex items-center justify-center overflow-hidden p-2 transition-all hover:scale-105">
              <ProviderIcon
                src={tool.image}
                alt={tool.name}
                size={48}
      ...
      >
        <path
          ...
          fill="none"
          stroke={activeFlow === 0 ? "var(--color-primary)" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 1 ? "var(--color-primary)" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 2 ? "var(--color-primary)" : "rgb(75, 85, 99)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 3 ? "var(--color-primary)" : "rgb(75, 85, 99)"}
        ...
        {PROVIDERS.map((provider, idx) => (
          <div
            ...
            className={`px-4 py-2 rounded-lg ${provider.color} ${provider.textColor} flex items-center justify-center font-bold text-xs shadow-lg hover:scale-110 transition-all cursor-help min-w-[140px] ${
              activeFlow === idx ? "landing-brand-ring scale-110" : ""
            }`}
            title={provider.name}
          >
            {provider.name}
